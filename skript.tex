...
Fehlerfunktional $E(\rharp{a}) = g^2$, $E\colon \R^m \to \R$
...
Setze $a^{(n+1)} = a^{(n)} + \delta^{(n)}$

Zu lösen $D^T \left[ g\right(a^{(n)}\left) \right] D\left[g\left(a^{(n)}\right)\right] \delta^{(n)} = - D^T\left[g\left(a^{(n)}\right)\right] g\left(a^{(n)}\right)$
